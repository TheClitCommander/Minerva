#!/usr/bin/env python3
"""
Enhanced WebSocket Client for Minerva

This client implements multiple authentication strategies and session initialization
patterns to solve the 403 Forbidden errors when connecting to Minerva.
"""
import os
import sys
import time
...
class EnhancedMinervaClient:
    """
    Enhanced client for connecting to Minerva WebSocket server with
    multiple authentication methods and fallback strategies.
    """
...
# Helper function to test connection with different authentication methods
def test_minerva_connection(url="http://localhost:5000"):
    ...
    # Create client
    client = EnhancedMinervaClient(url=url, timeout=20)
    
    # Test EIO version 3 with auto-discovery
    logger.info("🔄 Testing EIO v3 with auto-discovery")
    if client.connect(discover_auth=True, eio_version=3):
        logger.info("✅ Connected with EIO v3 and auto-discovery")
        # Send a test message
        response = client.send_message("Hello, Minerva!", wait_for_response=True)
        if response:
    ...
    # Test EIO version 4 with auto-discovery
    logger.info("🔄 Testing EIO v4 with auto-discovery")
    if client.connect(discover_auth=True, eio_version=4):
        logger.info("✅ Connected with EIO v4 and auto-discovery")
        # Send a test message
        response = client.send_message("Hello, Minerva!", wait_for_response=True)
        if response:
            logger.info("✅ Message sent and response received")
    ...
    # Test with basic auth if credentials are available
    if os.environ.get('MINERVA_USERNAME') and os.environ.get('MINERVA_PASSWORD'):
        logger.info("🔄 Testing with basic auth")
        client.set_authentication("basic", {
            "username": os.environ.get('MINERVA_USERNAME'),
            "password": os.environ.get('MINERVA_PASSWORD')
        })
        if client.connect(discover_auth=False):
            logger.info("✅ Connected with basic auth")
            # Send a test message
            response = client.send_message("Hello, Minerva!", wait_for_response=True)
            if response:
                logger.info("✅ Message sent and response received")
    ...
    # Test with custom Minerva headers
    logger.info("🔄 Testing with custom Minerva headers")
    client.set_authentication("custom", {
        "headers": {
            "X-Minerva-Session-ID": f"minerva_session_{uuid.uuid4().hex[:8]}",
            "X-Minerva-Client-Type": "websocket_client",
            "X-Minerva-Transaction-ID": f"tx_{int(time.time())}",
            "X-Minerva-Model-Request": "default"
        }
    })
...
if __name__ == "__main__":
    ...
    url = input("Enter Minerva server URL [http://localhost:5000]: ").strip() or "http://localhost:5000"
